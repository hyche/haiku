...
Volume::Mount(const char* deviceName, uint32 flags)
{
	...
	TRACE("Volume::Mount(): Found extent root: %" B_PRIu64 "\n",
		root->Logical());
	fExtentTree = new(std::nothrow) BTree(this, root->Logical());
	free(root);
	if (fExtentTree == NULL)
		return B_NO_MEMORY;
	...
	fFSTree = new(std::nothrow) BTree(this, root->Logical());
	free(root);
	if (fFSTree == NULL)
		return B_NO_MEMORY;
	...
	TRACE("Volume::Mount(): Found dev root: %" B_PRIu64 "\n",
		root->Logical());
	fDevTree = new(std::nothrow) BTree(this, root->Logical());
	free(root);
	if (fDevTree == NULL)
		return B_NO_MEMORY;
	...
	TRACE("Volume::Mount(): Found checksum root: %" B_PRIu64 "\n",
		root->Logical());
	fChecksumTree = new(std::nothrow) BTree(this, root->Logical());
	free(root);
