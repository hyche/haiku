...
struct btrfs_header {
	...
	uint32	item_count;
	uint8	level;
	uint64 Logical() const { return B_LENDIAN_TO_HOST_INT64(logical_address); }
...
struct btrfs_block_group {
	...
	uint64 UsedSpace() const { return used_space; }
	uint64 ChunkObjectID() const { return chunk_object_id; }
	uint64 Flags() const { return flags; }
} _PACKED;
